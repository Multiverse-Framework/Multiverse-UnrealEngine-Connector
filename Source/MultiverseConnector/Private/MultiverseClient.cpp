...
{
	if (const AStaticMeshActor* StaticMeshActor = Cast<AStaticMeshActor>(Object.Key))
	{
	...
	else if (const ASkeletalMeshActor* SkeletalMeshActor = Cast<ASkeletalMeshActor>(Object.Key))
	{
		TArray<TSharedPtr<FJsonValue>> AttributeJsonArray;
		for (const EAttribute& Attribute : Object.Value.Attributes)
		{
		...
		const FString ObjectName = Object.Value.ObjectName;
		CachedActors.Add(ObjectName, Object.Key);
		MetaDataJson->SetArrayField(ObjectName, AttributeJsonArray);
		...
		if (USkeletalMeshComponent* SkeletalMeshComponent = SkeletalMeshActor->GetSkeletalMeshComponent())
		{
			if (UMultiverseAnim* MultiverseAnim = Cast<UMultiverseAnim>(SkeletalMeshComponent->GetAnimInstance()))
			{
				...
				for (const FName& BoneName : BoneNames)
				{
					...
					if (((BoneNameStr.EndsWith(TEXT("_revolute_bone")) && BoneNameStr.RemoveFromEnd(
								TEXT("_revolute_bone"))) ||
							(BoneNameStr.EndsWith(TEXT("_continuous_bone")) && BoneNameStr.RemoveFromEnd(
								TEXT("_continuous_bone")))) &&
					...
					else if ((BoneNameStr.EndsWith(TEXT("_prismatic_bone")) && BoneNameStr.RemoveFromEnd(
							TEXT("_prismatic_bone"))) &&
			...
			{
				UE_LOG(LogMultiverseClient, Warning, TEXT("SkeletalMeshActor %s does not contain a MultiverseAnim."),
				       *Object.Value.ObjectName)
			}
		}
		else
		{
			UE_LOG(LogMultiverseClient, Warning, TEXT("SkeletalMeshActor %s does not contain a UStaticMeshComponent."),
			       *Object.Value.ObjectName)
		}
	}
}
...
static void BindDataArray(TArray<TPair<FString, EAttribute>>& DataArray,
                          const TPair<AActor*, FAttributeContainer>& Object)
{
	if (Cast<AStaticMeshActor>(Object.Key))
	{
		TArray<TSharedPtr<FJsonValue>> AttributeJsonArray;
		for (const EAttribute& Attribute : Object.Value.Attributes)
		{
	...
	else if (const ASkeletalMeshActor* SkeletalMeshActor = Cast<ASkeletalMeshActor>(Object.Key))
	{
		...
		if (USkeletalMeshComponent* SkeletalMeshComponent = SkeletalMeshActor->GetSkeletalMeshComponent())
		{
			if (Cast<UMultiverseAnim>(SkeletalMeshComponent->GetAnimInstance()))
			{
				TArray<FName> BoneNames;
				SkeletalMeshComponent->GetBoneNames(BoneNames);
				BoneNames.Sort([](const FName& BoneNameA, const FName& BoneNameB)
		...
		{
			return DataB.Key.Compare(DataA.Key) > 0 || (DataB.Key.Compare(DataA.Key) == 0 && DataB.Value > DataA.
				Value);
		});
	}
}
...
void FMultiverseClient::Init(const FString& ServerHost, const FString& ServerPort,
                             ...
                             UWorld* InWorld)
{
	...
	const FString ServerSocketAddr = ServerHost + ":" + ServerPort;
...
UMaterial* FMultiverseClient::GetMaterial(const FLinearColor& Color) const
{
	...
	return Cast<UMaterial>(StaticLoadObject(UMaterial::StaticClass(), nullptr,
	                                        *(TEXT("Material'/MultiverseConnector/Assets/Materials/") + ColorName +
		                                        TEXT(".") + ColorName + TEXT("'"))));
}
...
bool FMultiverseClient::compute_response_meta_data()
{
	...
	const FString ResponseMetaDataString(response_meta_data_str.c_str());
	const TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(ResponseMetaDataString);
...
void FMultiverseClient::compute_request_buffer_sizes(size_t& req_send_buffer_size,
                                                     size_t& req_receive_buffer_size) const
{
	...
	for (TPair<FString, size_t>& RequestBufferSize : RequestBufferSizes)
	{
		...
		for (const TPair<FString, TSharedPtr<FJsonValue>>& SendObjectJson : RequestMetaDataJson->GetObjectField(
			     RequestBufferSize.Key)->Values)
		{
			...
			for (const TSharedPtr<FJsonValue>& SendObjectAttribute : SendObjectJson.Value->AsArray())
			{
				...
				{
					RequestBufferSize.Value += AttributeDataMap[AttributeStringMap[SendObjectAttribute->AsString()]].
						Num();
				}
			}
		}
...
void FMultiverseClient::compute_response_buffer_sizes(size_t& res_send_buffer_size,
                                                      size_t& res_receive_buffer_size) const
{
	...
	for (TPair<FString, size_t>& ResponseBufferSize : ResponseBufferSizes)
	{
		...
		for (const TPair<FString, TSharedPtr<FJsonValue>>& ReceiveObject : ResponseMetaDataJson->GetObjectField(
			     ResponseBufferSize.Key)->Values)
		{
			for (const TPair<FString, TSharedPtr<FJsonValue>>& ReceiveObjectData : ReceiveObject.Value->AsObject()->
			     Values)
			{
...
bool FMultiverseClient::init_objects(bool from_server)
{
	...
	{
		SendObjects.ValueSort([](const FAttributeContainer& AttributeContainerA,
		                         const FAttributeContainer& AttributeContainerB)
		{
	...
	{
		ReceiveObjects.ValueSort([](const FAttributeContainer& AttributeContainerA,
		                            const FAttributeContainer& AttributeContainerB)
		{
			return AttributeContainerB.ObjectName.Compare(AttributeContainerA.ObjectName) > 0;
		});
	...
	for (const TPair<AActor*, FAttributeContainer>& ReceiveObject : ReceiveObjects)
	{
		...
		if (AStaticMeshActor* StaticMeshActor = Cast<AStaticMeshActor>(ReceiveObject.Key))
		{
			...
			{
				UE_LOG(LogMultiverseClient, Warning,
				       TEXT("StaticMeshActor %s in ReceiveObjects has None StaticMeshComponent."),
				       *ReceiveObject.Value.ObjectName)
				continue;
			}
			if (!StaticMeshComponent->IsSimulatingPhysics())
			{
				UE_LOG(LogMultiverseClient, Warning, TEXT("StaticMeshActor %s has disabled physics, enabling physics."),
				       *ReceiveObject.Value.ObjectName)
			...
			AActor* ReceiveObjectRef = World->SpawnActor<AStaticMeshActor>(
				AStaticMeshActor::StaticClass(), FTransform(), SpawnParams);
			if (ReceiveObjectRef == nullptr)
			{
				UE_LOG(LogMultiverseClient, Error, TEXT("Failed to spawn StaticMeshActor %s"),
				       *SpawnParams.Name.ToString())
				continue;
			}
			...
			PhysicsConstraint->SetConstrainedComponents(StaticMeshComponentRef, NAME_None, StaticMeshComponent,
			                                            NAME_None);
		...
		else if (Cast<ASkeletalMeshActor>(ReceiveObject.Key))
		{
...
void FMultiverseClient::bind_request_meta_data()
{
	...
	for (const TPair<AActor*, FAttributeContainer>& ReceiveObjectRef : ReceiveObjectRefs)
	{
		...
		BindMetaData(RequestMetaDataJson->GetObjectField(TEXT("receive")), ReceiveObjectRef, CachedActors,
		             CachedBoneNames);
	}

	FString RequestMetaDataString;
	const TSharedRef<TJsonWriter<TCHAR>> Writer = TJsonWriterFactory<TCHAR>::Create(&RequestMetaDataString);
	FJsonSerializer::Serialize(RequestMetaDataJson.ToSharedRef(), Writer, true);

	request_meta_data_str.clear();
	for (size_t i = 0; i < (RequestMetaDataString.Len() + 127) / 128; i++)
	// Split string into multiple substrings with a length of 128 characters or less
	{
...
void FMultiverseClient::bind_response_meta_data()
{
	...
	const TSharedPtr<FJsonObject> ResponseSendObjects = ResponseMetaDataJson->GetObjectField(TEXT("send"));
			...
			case EAttribute::Position:
				{
					...
					TArray<TSharedPtr<FJsonValue>> ObjectPosition = ResponseSendObjects->
						GetArrayField(TEXT("position"));
			...
			case EAttribute::Quaternion:
				{
					...
					TArray<TSharedPtr<FJsonValue>> ObjectQuaternion = ResponseSendObjects->GetArrayField(
						TEXT("quaternion"));
			...
			case EAttribute::JointRvalue:
				{
					...
					TArray<TSharedPtr<FJsonValue>> JointRvalue = ResponseSendObjects->GetArrayField(
						TEXT("joint_rvalue"));
					if (JointRvalue.Num() != 1)
					{
						continue;
					}
					CachedBoneNames[SendData.Key].Key->JointPoses[CachedBoneNames[SendData.Key].Value].SetRotation(
						FQuat(FRotator(JointRvalue[0]->AsNumber(), 0.f, 0.f)));
					break;
				}
			...
			case EAttribute::JointTvalue:
				{
					...
					TArray<TSharedPtr<FJsonValue>> JointTvalue = ResponseSendObjects->GetArrayField(
						TEXT("joint_tvalue"));
					...
					CachedBoneNames[SendData.Key].Key->JointPoses[CachedBoneNames[SendData.Key].Value].SetTranslation(
						FVector(0.f, JointTvalue[0]->AsNumber(), 0.f));
					break;
				}
...
void FMultiverseClient::bind_send_data()
{
	...
	for (const TPair<FString, EAttribute>& SendData : SendDataArray)
	{
		...
		else if (CachedBoneNames.Contains(SendData.Key))
		{
			...
			case EAttribute::JointRvalue:
				{
					const FQuat JointQuaternion = CachedBoneNames[SendData.Key].Key->JointPoses[CachedBoneNames[SendData
						.Key].Value].GetRotation();
			...
			case EAttribute::JointTvalue:
				{
					const FVector JointPosition = CachedBoneNames[SendData.Key].Key->JointPoses[CachedBoneNames[SendData
						.Key].Value].GetTranslation();
...
{
	const double* receive_buffer_addr = receive_buffer + 1;
			...
			case EAttribute::JointRvalue:
				{
					...
					CachedBoneNames[ReceiveData.Key].Key->JointPoses[CachedBoneNames[ReceiveData.Key].Value].
						SetRotation(FQuat(FRotator(JointRvalue, 0.f, 0.f)));
					break;
				}
			...
			case EAttribute::JointTvalue:
				{
					...
					CachedBoneNames[ReceiveData.Key].Key->JointPoses[CachedBoneNames[ReceiveData.Key].Value].
						SetTranslation(FVector(0.f, JointTvalue, 0.f));
					break;
				}
